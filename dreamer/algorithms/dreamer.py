...
class Dreamer:
    def __init__(
        ...
        self.model_optimizer = torch.optim.Adam(self.model_params, lr=self.config.model_learning_rate)
    ...
    def save_model(self, itr: int):
        ...
        path = self.config.save_path + "dreamer" + str(itr) + ".pth"
    ...
    def load_model(self, path: str):
        ...
        # model
        self.rssm.load_state_dict(checkpoint["rssm_state_dict"], map_location=self.device)
        self.encoder.load_state_dict(checkpoint["encoder_state_dict"], map_location=self.device)
        self.decoder.load_state_dict(checkpoint["decoder_state_dict"], map_location=self.device)
        self.reward_predictor.load_state_dict(checkpoint["reward_predictor_state_dict"], map_location=self.device)
        self.actor.load_state_dict(checkpoint["actor_state_dict"], map_location=self.device)
        self.critic.load_state_dict(checkpoint["critic_state_dict"], map_location=self.device)
        if hasattr(self, "continue_predictor") and "continue_predictor_state_dict" in checkpoint.keys():
            self.continue_predictor.load_state_dict(
                checkpoint["continue_predictor_state_dict"], map_location=self.device
            )

        # optimizer
        self.model_optimizer.load_state_dict(checkpoint["model_optimizer_state_dict"], map_location=self.device)
        self.actor_optimizer.load_state_dict(checkpoint["actor_optimizer_state_dict"], map_location=self.device)
        self.critic_optimizer.load_state_dict(checkpoint["critic_optimizer_state_dict"], map_location=self.device)
